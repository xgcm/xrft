...
def test_detrend_1D(array_dims, array_shape, detrend_dim, chunks, detrend_type, trend_amplitude, linear_error):
...
@pytest.mark.parametrize('detrend_type', [None, 'constant', 'linear'])
@pytest.mark.parametrize('trend_amplitude', [{'x': 0.1, 'y': 0.1}, {'x': 10., 'y': 0.01}])
def test_detrend_2D(array_dims, array_shape, chunks, detrend_type, trend_amplitude):
    da_original = noise(array_dims, array_shape)
    da_trend = da_original + trend_amplitude['x'] * da_original['x'] + trend_amplitude['y'] + da_original['x']
    if chunks:
        da_trend = da_trend.chunk(chunks)
