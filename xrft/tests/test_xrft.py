...
class TestDFTImag(object):
    def test_dft_1d(self, test_data_1d):
        """Test the discrete Fourier transform function on one-dimensional
         data."""
    ...
    def test_dft_3d_dask(self, dask):
        ...
        if dask:
            ...
            npt.assert_almost_equal(
                daft.values, np.fft.fftn(da_prime, axes=[0])
            )
            npt.assert_array_equal(
                daft.values,
    ...
    def test_dft_4d(self):
        ...
        da_prime = xrft.detrendn(da[:, 0].values, [0, 1, 2])
        npt.assert_almost_equal(
            xrft.dft(
                da[:, 0].drop("z"),
                dim=["time", "y", "x"],
...
def test_chunks_to_segments():
    ...
    npt.assert_almost_equal(
        ft.values, dsar.fft.fftn(data, axes=[1]), decimal=7
    )
    ft = xrft.dft(
        da.chunk({"y": 16, "x": 16}),
        dim=["y", "x"],
    ...
    npt.assert_almost_equal(
        ft.values, dsar.fft.fftn(data, axes=[2, 4]), decimal=7
    )
    ...
    npt.assert_almost_equal(ps.values, (ft * np.conj(ft)).real.values)
    da2 = xr.DataArray(
        np.random.rand(N, N, N),
        dims=["time", "y", "x"],
        coords={"time": range(N), "y": range(N), "x": range(N)},
...
def test_dft_nocoords():
    ...
    data = xr.DataArray(
        np.random.random([20, 30, 100]), dims=["time", "lat", "lon"]
    )
    xrft.dft(data, dim=["time"])
    xrft.power_spectrum(data, dim=["time"])
...
class TestSpectrum(object):
    ...
    def test_power_spectrum(self, dask):
        ...
        da = xr.DataArray(
            ...
            coords={
                "time": np.array(
                    ["2019-04-18", "2019-04-19"], dtype="datetime64"
                ),
                "y": range(N),
                "x": range(N),
            },
        ...
        daft = xrft.dft(
            da, dim=["y"], real="x", detrend="constant", window=True
        )
        npt.assert_almost_equal(ps.values, np.real(daft * np.conj(daft)))

        # Normalized
        ps = xrft.power_spectrum(
            da, dim=["y", "x"], window=True, detrend="constant"
        )
    ...
    def test_cross_spectrum(self, dask):
        ...
        daft = xrft.dft(
            da, dim=dim, shift=True, detrend="constant", window=True
        )
        daft2 = xrft.dft(
            da2, dim=dim, shift=True, detrend="constant", window=True
        )
        cs = xrft.cross_spectrum(
...
def test_parseval():
    ...
    npt.assert_almost_equal(
        ps.values.sum(),
        (np.asarray(delta_x).prod() * ((da_prime * window) ** 2).sum()),
        decimal=5,
    )
    ...
    npt.assert_almost_equal(
        ...
        (
            ...
            * ((da_prime * window) * (da2_prime * window)).sum()
        ),
        decimal=5,
    )
    ...
    ps = xrft.power_spectrum(
        d3d, dim=["x", "y"], window=True, detrend="linear"
    )
    npt.assert_almost_equal(
        ps[0].values.sum(),
        (
...
def _synthetic_field(N, dL, amp, s):
    ...
    N_2 = int(N / 2)
    phi_upper_right = (
        2.0 * np.pi * np.random.random((N_2 - 1, N_2 - 1)) - np.pi
    )
...
def test_isotropic_ps_slope(N=512, dL=1.0, amp=1e1, s=-3.0):
    ...
    theta = xr.DataArray(
        _synthetic_field(N, dL, amp, s),
        dims=["y", "x"],
        coords={"y": range(N), "x": range(N)},
    )
    iso_ps = xrft.isotropic_powerspectrum(
        theta, detrend="constant", density=True
    )
...
def test_isotropic_ps():
    ...
    npt.assert_almost_equal(
        np.ma.masked_invalid(iso_ps[:, :, 1:]).mask.sum(), 0.0
    )
...
def test_isotropic_cs():
    ...
    npt.assert_almost_equal(np.ma.masked_invalid(iso_cs[1:]).mask.sum(), 0.0)
    ...
    iso_cs = xrft.isotropic_crossspectrum(
        da, da2, dim=["y", "x"], window=True
    ).values
    npt.assert_almost_equal(
        np.ma.masked_invalid(iso_cs[:, :, 1:]).mask.sum(), 0.0
    )
